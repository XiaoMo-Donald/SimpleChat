...
(function($, window, document) {
    ...
    Plugin.prototype = {
        _init: function() {
            ...
            var emoji_container = '<div class="emoji_container" id="emoji_container_' + ix + '"><span class="emoji-tri-tip"></span>';
            ...
            for (var i = 0; i < groupLength; i++) {
                ...
                for (var j = 1; j <= maxNum; j++) {
                    ...
                    panel += '<li><a data-ix="' + ix + '" data-emoji_code="' + notation + '" data-index="' + index + '" title="' + (title && title[j] ? title[j] : '') + '"><img src="' + path + j + file + '"/></a></li>';
                    index++;
                }
                panel += '</ul></div>';
                emoji_content += panel;
            ...
            $(emoji_container).appendTo(this.$content.parent());
            let _timer = setTimeout(() => {
                clearTimeout(_timer);
                $('#emoji_container_' + ix).mouseleave(function() {
                    that.hide();
                });
                // $('.emoji_container').mouseleave(function() {
                //     that.hide();
                // });
            }, 10);
            ...
            $('.chat-window').off('click').on({
                'click': function(e) {
                    ...
                    var code,
                        ...
                        imgSrc,
                        insertHtml;
                    if (target.toString() === $(btn)[0].toString()) {
                        field = $(target).parent().parent().parent().parent().parent().parent().find('.send-message-text-input');
                        that.$content = field;
                        ix = $(target).data('index');
                    }
                    if (target.toString() === $(btn)[0].toString()) {
                        $('#emoji_container_' + ix)[that.toggleFunc]();
                        that.$content.focus();
                    } else if ($(target).parents('#emoji_container_' + ix).length > 0) {
                        // ix = $(target).data('ix') || $(target).parent().data('ix');
